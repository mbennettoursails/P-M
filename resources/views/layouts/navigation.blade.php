            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 sm:space-x-4">
                ...
                @auth
                    @if(Route::has('notifications.index'))
                        @php
                            $unreadCount = 0;
                            try {
                                if (class_exists(\App\Models\ProposalNotification::class)) {
                                    $unreadCount = \App\Models\ProposalNotification::where('user_id', Auth::id())
                                        ->whereNull('read_at')
                                        ->count();
                                }
                            } catch (\Exception $e) {
                        ...
                        @endphp
                        <a href="{{ route('notifications.index') }}" class="relative p-2 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
                ...
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        ...
                        @if(Route::has('admin.dashboard') && (Auth::user()->is_admin ?? false))
                ...
                @auth
                    @if(Route::has('notifications.index'))
                        @php
                            ...
                            try {
                                if (class_exists(\App\Models\ProposalNotification::class)) {
                                    $mobileUnread = \App\Models\ProposalNotification::where('user_id', Auth::id())
                                        ->whereNull('read_at')
                                        ->count();
                                }
                            } catch (\Exception $e) {
                        ...
                        @endphp
                        <a href="{{ route('notifications.index') }}" class="relative p-2 text-gray-500 dark:text-gray-400 mr-2">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                            </svg>
                            @if($mobileUnread > 0)
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Route::has('decisions.create'))
                <x-responsive-nav-link :href="route('decisions.create')">
                    {{ __('New Proposal') }}
                </x-responsive-nav-link>
            @endif
        </div>
